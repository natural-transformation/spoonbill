...
Event gives you access to information from the client side. In this case, it is
values of the input fields. You can also get data from the event object using
`access.eventData`.

For simple field reads, `valueAs` and `checked` decode common DOM properties into
typed values.

[source,scala]
-----------------------------------
for {
  duration <- access.valueAs[Int](durationInput)
  enabled <- access.checked(enabledInput)
  _ <- access.transition(_.copy(duration = duration, enabled = enabled))
} yield ()
-----------------------------------

For event data, define a `Context.EventDataDecoder` for the application-specific
shape and use `eventDataAs`.

[source,scala]
-----------------------------------
case class KeyData(raw: String)

implicit val keyDataDecoder: Context.EventDataDecoder[KeyData] =
  Context.EventDataDecoder.instance(raw => Right(KeyData(raw)))

event("keydown") { access =>
  access.eventDataAs[KeyData].flatMap { keyData =>
    access.transition(_.copy(lastKey = keyData.raw))
  }
}
-----------------------------------
...
Use `afterRender` when an event handler needs to transition state and then run a
client-side effect against the rendered DOM.

[source,scala]
------------------------------
for {
  _ <- access.transition(_.copy(saved = true))
  _ <- access.afterRender { rendered =>
    rendered.focus(nameInput)
  }
} yield ()
------------------------------

`afterRender` waits for pending server-side rendering work before running the
effect. It is useful for operations such as focusing an element or setting a DOM
property after the element may have been replaced by a render diff.
...
------------------------------
libraryDependencies += "com.natural-transformation" %% "spoonbill-testkit" % "{version}" % Test
------------------------------
...
Use `StateStorage.fromRepository` when session state should be stored outside the
default in-memory or developer-mode storage.

[source,scala]
-------------------------------------------------------------------------------
val storage = StateStorage.fromRepository[Future, AppState](
  repositoryExists = key => repo.exists(key),
  loadSnapshot = key => repo.loadSnapshot(key),
  createSnapshot = (key, snapshot) => repo.create(key, snapshot),
  writeNode = (key, nodeId, bytes) => repo.write(key, nodeId, bytes),
  deleteNode = (key, nodeId) => repo.delete(key, nodeId),
  removeRepository = key => repo.remove(key)
)

val config = SpoonbillServiceConfig[Future, AppState, Any](
  stateLoader = StateLoader.default(AppState.empty),
  stateStorage = storage,
  document = render
)
-------------------------------------------------------------------------------

`StateManager.cached` and `StateManager.serialized` are also available for
lower-level custom storage implementations that only need reusable read, write,
delete, and snapshot mechanics.
...
[source,scala,subs="attributes"]
------------------------------
libraryDependencies += "com.natural-transformation" %% "spoonbill-slf4j" % "{version}"
------------------------------
...
[source,scala,subs="attributes"]
------------------------------
libraryDependencies += "com.natural-transformation" %% "spoonbill-akka" % "{version}"
------------------------------
...
[source,scala,subs="attributes"]
------------------------------
libraryDependencies += "com.natural-transformation" %% "spoonbill-pekko" % "{version}"
------------------------------
...
val spoonbillRoute = pekkoHttpService(config).apply(PekkoHttpServerConfig())
------------------------------

Then embed the route into your Pekko HTTP routes structure.
...
[source,scala,subs="attributes"]
------------------------------
libraryDependencies += "com.natural-transformation" %% "spoonbill-zio-http" % "{version}"
------------------------------
...
[source,scala,subs="attributes"]
------------------------------
libraryDependencies += "com.natural-transformation" %% "spoonbill-zio" % "{version}"
------------------------------
...
[source,scala,subs="attributes"]
------------------------------
libraryDependencies += "com.natural-transformation" %% "spoonbill-zio2" % "{version}"
------------------------------
...
[source,scala,subs="attributes"]
------------------------------
libraryDependencies += "com.natural-transformation" %% "spoonbill-monix" % "{version}"
------------------------------
...
[source,scala,subs="attributes"]
------------------------------
libraryDependencies += "com.natural-transformation" %% "spoonbill-ce2" % "{version}"
------------------------------
...
[source,scala,subs="attributes"]
------------------------------
libraryDependencies += "com.natural-transformation" %% "spoonbill-ce3" % "{version}"
------------------------------
...
[source,scala,subs="attributes"]
------------------------------
libraryDependencies += "com.natural-transformation" %% "spoonbill-zio-streams" % "{version}"
------------------------------
...
libraryDependencies += "com.natural-transformation" %% "spoonbill-fs2-ce2" % "{version}"
// For Fs2-3.* and Cats Effect 3:
libraryDependencies += "com.natural-transformation" %% "spoonbill-fs2-ce3" % "{version}"
------------------------------
