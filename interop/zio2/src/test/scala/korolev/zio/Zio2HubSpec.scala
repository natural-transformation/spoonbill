      ...
      suite("sequential publishers and subscribers")(
        ...
        test("with one publisher and two subscribers") {
          check(smallInt, Gen.listOf(smallInt)) { (n, as) =>
            for {
              ...
              subscriber1 <- ZIO.scoped {
                               hub
                                 ...
                                 .flatMap(subscription =>
                                   promise1.succeed(()) *> promise3.await *> ZIO.foreach(as.take(n))(_ =>
                                     subscription.pull()
                                   )
                                 )
                             }.fork
              subscriber2 <- ZIO.scoped {
                               hub
                                 .newStream()
                                 .flatMap(subscription =>
                                   promise2.succeed(()) *> promise3.await *> ZIO.foreach(as.take(n))(_ =>
                                     subscription.pull()
                                   )
                                 )
                             }.fork
            ...
            } yield assert(values1.flatten)(equalTo(as.take(n))) &&
        ...
        test("many to many") {
          check(smallInt, Gen.listOf(smallInt)) { (n, as) =>
            for {
              ...
              subscriber1 <- ZIO.scoped {
                               hub.newStream().flatMap { subscription =>
                                 promise1.succeed(()) *> ZIO.foreach((as ::: as).take(n * 2))(_ =>
                                   subscription.pull().some
                                 )
                               }
                             }.fork
              subscriber2 <- ZIO.scoped {
                               hub.newStream().flatMap { subscription =>
                                 promise2.succeed(()) *> ZIO.foreach((as ::: as).take(n * 2))(_ =>
                                   subscription.pull().some
                                 )
                               }
                             }.fork
