...
package object dsl {
  ...
  def request[F[_]: Effect, B](method: Request.Method, pq: PathAndQuery)(implicit
    bf: EmptyBodyFactory[B]
  ): F[Request[B]] = Effect[F].pure {
    val body = bf.emptyBody
    Request(method, pq, body.headers.toSeq, body.contentLength, body.content)
  }

  def request[F[_]: Effect, A, B](method: Request.Method, pq: PathAndQuery, source: A)(implicit
    bf: BodyFactory[F, A, B]
  ): F[Request[B]] = bf.mkBody(source).map { body =>
    Request(method, pq, body.headers.toSeq, body.contentLength, body.content)
  }

  def request[F[_]: Effect, A, B](method: Request.Method, pq: PathAndQuery, source: A, headers: Map[String, String])(
    implicit bf: BodyFactory[F, A, B]
  ): F[Request[B]] = bf.mkBody(source).map { body =>
    val allHeaders = body.headers ++ headers
