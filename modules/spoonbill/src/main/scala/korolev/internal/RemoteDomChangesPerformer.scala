...
import spoonbill.internal.Frontend.ModifyDomProcedure
import avocet.FastId
import avocet.impl.DiffRenderContext.ChangesPerformer
import scala.collection.mutable

private[spoonbill] class RemoteDomChangesPerformer extends ChangesPerformer {
  ...
  private def appendXmlNs(xmlNs: String): Unit =
    if (xmlNs eq avocet.XmlNs.html.uri) {
      buffer.append('0')
      buffer.append(',')
