...
import spoonbill.effect.{AsyncTable, Effect, Queue, Reporter, Scheduler, Stream}
...
object Frontend {
  ...
  private def extractPropertyTimeout: FiniteDuration =
    Option(System.getProperty("spoonbill.extractPropertyTimeoutMillis"))
      .flatMap(_.toLongOption)
      .map(_.millis)
      .getOrElse(DefaultExtractPropertyTimeout)

  private def extractEventDataTimeout: FiniteDuration =
    Option(System.getProperty("spoonbill.extractEventDataTimeoutMillis"))
      .flatMap(_.toLongOption)
      .map(_.millis)
