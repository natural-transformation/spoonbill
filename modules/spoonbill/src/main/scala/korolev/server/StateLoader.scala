...
import spoonbill.state.DeviceId
...
object StateLoader {
  ...
  /**
   * State depends on deviceId. Useful when you want to restore user
   * authorization.
   *
   * {{{
  ...
  /**
   * State depends on deviceId and HTTP-request. Second one could be None if
   * case when user reconnected to restarted application and state wasn't
   * restored.
   */
