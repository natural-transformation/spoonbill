...
import spoonbill.web.{FormData, MimeTypes}
import avocet.*
...
object Context {
  ...
  sealed abstract class Scope[F[_], S, AccessType, M] {
    ...
    type Transition      = spoonbill.Transition[S]
    ...
    type Document        = avocet.Document[Binding]
    type Node            = avocet.Document.Node[Binding]
    type Attr            = avocet.Document.Attr[Binding]
    ...
    /**
     * Schedules the transition with delay. For example it can be useful when
     * you want to hide something after timeout.
     */
    ...
    implicit final class ComponentDsl[CS: StateSerializer: StateDeserializer, P, E](component: Component[F, CS, P, E])(
      ...
      final class ComponentEntryWithEventHandlerDsl(p: P)
          extends ComponentEntry[F, S, M, CS, P, E](component, p, (_, _) => Effect[F].unit) {
  ...
  trait BaseAccess[F[_], S, M] {
    ...
    /**
     * Downloads form from the client. Useful when when you want to read big
     * amount of fields. Do not use this method for downloading files, however
     * it is possible.
     *
     * {{{
     * event(submit) { access =>
    ...
    /**
     * Download the selected file list from input appropriate to given element
     * id. Use this method carefully because all files are saving to RAM.
     */
    def downloadFiles(id: ElementId): F[List[(FileHandler, Bytes)]]

    /**
     * Same as [[downloadFiles]] but for stream mode. The method is useful when
     * user want to upload very large files list which is problematic to keep in
     * memory (especially when count of users is more than one).
     */
    ...
    def uploadFile(
      ...
      size: Option[Long],
      mimeType: String = MimeTypes.`application/octet-stream`
    ): F[Unit]
    ...
    /**
     * Applies asynchronous transition to current state. All transitions will
     * wait until this transition will executed.
     *
     * NOTE: Do not use this method id you work with effects which take lot of
     * time for execution. It's may lead to hanging of your app.
     * @return
     */
    ...
    /**
     * Execute arbitrary JavaScript code on client and get stringified JSON
     * back.
     * {{{
     * for {
  ...
  trait EventAccess[F[_], S, M] {
    ...
    /**
     * Gives json with string, number and boolean fields of object of the event
     * happened in current render phase. Note that is expensive operation which
  ...
  class MappedAccess[F[_]: Effect, S1, SN, E](self: Access[F, S1, E], lens: Lens[S1, SN]) extends Access[F, SN, E] {
    ...
    def uploadFile(name: String, stream: Stream[F, Bytes], size: Option[Long], mimeType: String): F[Unit] =
      self.uploadFile(name, stream, size, mimeType)
    ...
    def transition(f: spoonbill.Transition[SN]): F[Unit]              = self.transition(this.lens)(f)
    ...
    def transitionAsync[B](lens: Lens[SN, B])(f: TransitionAsync[F, B]): F[Unit] =
      self.transitionAsync(this.lens ++ lens)(f)
  ...
  final case class FileHandler(fileName: String, size: Long)(private[spoonbill] val elementId: ElementId)
    ...
    def createInstance(
      ...
      new ComponentInstance(
        ...
        createMiscProxy = (rc, k) =>
          new StatefulRenderContext[Binding[F, CS, E]] {
  ...
  ) extends Binding[F, S, M]

  final case class Delay[F[_], S, M](duration: FiniteDuration, effect: Access[F, S, M] => F[Unit])
      extends Binding[F, S, M]
